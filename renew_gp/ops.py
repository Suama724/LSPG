...
class Operator:
    def __init__(self, function, name, arity,
        ...
        '''
        function: 实际的callable函数
        name: 算子名称
        arity: 参数个数, 注意对于aggregate函数该项无效
        aggregate函数: 如mean, sum
        commutative: 满足交换律(优化用)
        protected: 需要特殊处理(log(负数), sqrt(负数)等)
        trigonometric: 三角函数
        '''
...
def make_operator(function, name, arity, **kwargs):
    if not isinstance(arity, int):
        raise ValueError("arity should be an integer")
...
def _sum_func(x): return np.sum(x, axis=0)
def _prod_func(x): return np.prod(x, axis=0)
def _mean_func(x): return np.mean(x, axis=0)
...
# arity=0 作为标记
sum_op  = Operator(_sum_func,  'sum',  0, is_aggregate=True)
prod_op = Operator(_prod_func, 'prod', 0, is_aggregate=True)
mean_op = Operator(_mean_func, 'mean', 0, is_aggregate=True)
