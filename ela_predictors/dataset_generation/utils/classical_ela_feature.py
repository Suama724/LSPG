...
def calculate_ela_conv(
    ...
    try:
        ...
        ok_shape = Y_new_raw.size == ela_conv_nsample and (
            Y_new_raw.ndim == 1 or (Y_new_raw.ndim == 2 and Y_new_raw.shape[1] == 1)
        )
        if ok_shape:
        ...
        Y_new_real = np.array([_conv_single_eval(f, X_new[k]) for k in range(ela_conv_nsample)], dtype=np.float64)
...
def calculate_ela_meta(X: np.ndarray, y: np.ndarray) -> Dict[str, Union[int, float]]:
    ...
    lin_w_interact_adj_r2 = 1 - (1 - model.score(X_interact, y)) * (n_samples - 1) / (
        n_samples - X_interact.shape[1] - 1
    )

    X_squared = np.hstack([X, X**2])
    model = linear_model.LinearRegression()
    model.fit(X_squared, y)
    quad_simple_adj_r2 = 1 - (1 - model.score(X_squared, y)) * (n_samples - 1) / (
        n_samples - X_squared.shape[1] - 1
    )
    ...
    for i in range(n_sq):
        ...
        if n_remaining > 0:
            X_quad_interact[:, col : col + n_remaining] = X_squared[:, i : i + 1] * X_squared[:, i + 1 :]
            col += n_remaining
    model = linear_model.LinearRegression()
    model.fit(X_quad_interact, y)
    quad_w_interact_adj_r2 = 1 - (1 - model.score(X_quad_interact, y)) * (n_samples - 1) / (
        n_samples - X_quad_interact.shape[1] - 1
    )
...
def calculate_information_content(
    ...
    if ic_sorting == "random":
        ...
        for i in range(1, n_samples):
            ...
            if next_c:
                ...
                idx_prev = np.where(indices[permutation[i - 1]] == current)[0][0]
                dists[i] = distances[permutation[i - 1], idx_prev]
            else:
    ...
    ratio = np.ediff1d(y_perm) / d
    abs_ratio = np.abs(ratio)

    psi_eps = np.where(abs_ratio[:, None] < epsilon, 0, np.sign(ratio)[:, None]).astype(np.int8)
    a = psi_eps[:-1, :]
    b = psi_eps[1:, :]
    ...
    probs = np.stack([p_m1_0, p_m1_1, p_0_m1, p_0_1, p_1_m1, p_1_0], axis=0)
    ...
    return {
        "ic.h_max": float(np.max(H)),
        "ic.eps_s": eps_s,
        "ic.eps_max": float(np.median(epsilon[H == np.max(H)])),
        "ic.eps_ratio": eps05,
        "ic.m0": float(m0),
