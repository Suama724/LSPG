...
class F3(BasicProblemBBOB):
    def __init__(self, dim, shift, rotate, bias, lb, ub):
        self.scales = (10.0**0.5) ** np.linspace(0, 1, dim)
        super().__init__(dim, shift, rotate, bias, lb, ub)
    ...
    def func(self, x):
        ...
        return 10.0 * (self.dim - np.sum(np.cos(2.0 * np.pi * z), axis=-1)) + np.sum(z**2, axis=-1) + self.bias
...
class F5(BasicProblemBBOB):
    ...
    def func(self, x):
        ...
        z[exceed_bound] = np.sign(z[exceed_bound]) * self.ub
...
class F13(BasicProblemBBOB):
    ...
    def func(self, x):
        self.FES += x.shape[0]
        z = shift_rotate(x, self.shift, self.rotate)
        return z[:, 0] ** 2.0 + 100.0 * np.sqrt(np.sum(z[:, 1:] ** 2.0, axis=-1)) + self.bias
...
class F16(BasicProblemBBOB):
    ...
    def func(self, x):
        ...
        return (
            10 * np.power(np.mean(np.sum(self.aK * np.cos(np.matmul(2 * np.pi * (z[:, :, None] + 0.5), self.bK[None, :])), axis=-1), axis=-1) - self.f0, 3)
            + 10 / self.dim * bbob_pen_func(x, self.ub)
            + self.bias
        )
...
class F20(BasicProblemBBOB):
    def __init__(self, dim, shift, rotate, bias, lb, ub):
        shift = 0.5 * 4.2096874633 * np.random.choice([-1.0, 1.0], size=dim)
        BasicProblemBBOB.__init__(self, dim, shift, rotate, bias, lb, ub)
...
class F23(BasicProblemBBOB):
    def __init__(self, dim, shift, rotate, bias, lb, ub):
        scales = (100.0**0.5) ** np.linspace(0, 1, dim)
        rotate = np.matmul(np.matmul(gen_rotate_matrix_qr(dim), np.diag(scales)), rotate)
        BasicProblemBBOB.__init__(self, dim, shift, rotate, bias, lb, ub)
...
class F24(BasicProblemBBOB):
    def __init__(self, dim, shift, rotate, bias, lb, ub):
        self.mu0 = 2.5 / 5 * ub
        shift = np.random.choice([-1.0, 1.0], size=dim) * self.mu0 / 2
    ...
    def func(self, x):
        self.FES += x.shape[0]
        x_hat = 2.0 * np.sign(self.shift) * x
        z = np.matmul(x_hat - self.mu0, self.rotate.T)
        s = 1.0 - 1.0 / (2.0 * np.sqrt(self.dim + 20.0) - 8.2)
        mu1 = -np.sqrt((self.mu0**2 - 1) / s)
        return np.minimum(np.sum((x_hat - self.mu0) ** 2.0, axis=-1), self.dim + s * np.sum((x_hat - mu1) ** 2.0, axis=-1)) + 10.0 * (self.dim - np.sum(np.cos(2.0 * np.pi * z), axis=-1)) + 1e4 * bbob_pen_func(x, self.ub) + self.bias
...
def build_instance(
    ...
    lb = -1 * upperbound
    if shifted:
        shift = 0.8 * (rng.random(dim) * (ub - lb) + lb)
    else:
        shift = np.zeros(dim)
    if rotated:
        ...
        H = np.eye(dim)
    if biased:
        bias = int(rng.randint(1, 26)) * 100
    else:
        bias = 0

    cls = globals().get(f"F{int(meta_func_id)}")
    if cls is None:
        raise ValueError(f"Unsupported BBOB function id: {meta_func_id}")
    return cls(dim=dim, shift=shift, rotate=H, bias=bias, lb=lb, ub=ub)
