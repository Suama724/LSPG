...
class NoisyProblem(BasicProblemBBOB):
    ...
    def boundaryHandling(self, x):
        return 100.0 * bbob_pen_func(x, self.ub)
...
class UniformNoisyProblem(NoisyProblem):
    def noisy(self, ftrue):
        ...
        fnoisy_unbiased = (
            ftrue_unbiased
            * (np.random.rand(*ftrue_unbiased.shape) ** self.uniform_beta)
            * np.maximum(
                1.0,
                (1e9 / (ftrue_unbiased + 1e-99))
                ** (self.uniform_alpha * (0.49 + 1.0 / self.dim) * np.random.rand(*ftrue_unbiased.shape)),
            )
        )
        return np.where(ftrue_unbiased >= 1e-8, fnoisy_unbiased + bias + 1.01 * 1e-8, ftrue)
...
class CauchyNoisyProblem(NoisyProblem):
    def noisy(self, ftrue):
        ...
        bias = self.optimum
        ftrue_unbiased = ftrue - bias
        fnoisy_unbiased = ftrue_unbiased + self.cauchy_alpha * np.maximum(
            0.0,
            1e3
            + (np.random.rand(*ftrue_unbiased.shape) < self.cauchy_p)
            * np.random.randn(*ftrue_unbiased.shape)
            / (np.abs(np.random.randn(*ftrue_unbiased.shape)) + 1e-199),
        )
        return np.where(ftrue_unbiased >= 1e-8, fnoisy_unbiased + bias + 1.01 * 1e-8, ftrue)
...
class Step_Ellipsoidal(BasicProblemBBOB):
    def __init__(self, dim, shift, rotate, bias, lb, ub):
        scales = (10.0**0.5) ** np.linspace(0, 1, dim)
    ...
    def func(self, x):
        ...
        rounded_z = np.where(np.abs(z_hat) > 0.5, np.floor(0.5 + z_hat), np.floor(0.5 + 10.0 * z_hat) / 10.0)
        z = np.matmul(rounded_z, self.Q_rotate_T)
        main_term = np.sum(self.weights * (z**2), axis=-1)
        first_dim_term = np.abs(z_hat[:, 0]) / 1e4
        return 0.1 * np.maximum(first_dim_term, main_term) + self.boundaryHandling(x) + self.bias
...
class Rosenbrock(BasicProblemBBOB):
    ...
    def func(self, x):
        self.FES += x.shape[0]
        z = max(1.0, self.dim**0.5 / 8.0) * shift_rotate(x, self.shift, self.rotate) + 1
...
class Scaffer(BasicProblemBBOB):
    ...
    def func(self, x):
        ...
        return (self.inv_dim_minus_1 * np.sum(inner_term, axis=-1)) ** 2 + self.boundaryHandling(x) + self.bias
...
class Composite_Grie_rosen(BasicProblemBBOB):
    ...
    def __init__(self, dim, shift, rotate, bias, lb, ub):
        ...
        shift = np.matmul(0.5 * np.ones(dim) / (scale**2.0), self.linearTF)
        super().__init__(dim, shift, rotate, bias, lb, ub)
    ...
    def func(self, x):
        ...
        return self.factor + self.factor * np.sum(s / 4000.0 - np.cos(s), axis=-1) / (self.dim - 1.0) + self.bias + self.boundaryHandling(x)
...
class Gallagher(BasicProblemBBOB):
    ...
    def __init__(self, dim, shift, rotate, bias, lb, ub):
        ...
        elif self.n_peaks == 21:
            opt_shrink = 0.98
            global_opt_alpha = 1e6
        else:
            raise ValueError(f"{self.n_peaks} peaks Gallagher is not supported yet.")

        self.y = opt_shrink * (np.random.rand(self.n_peaks, dim) * (ub - lb) + lb)
        self.y[0] = shift * opt_shrink
        shift = self.y[0]
        ...
        self.w = np.insert(np.linspace(1.1, 9.1, self.n_peaks - 1), 0, 10.0)
