...
def gen_rotate_matrix_householder(dim, random_state=None):
...
def gen_rotate_matrix_qr(dim, random_state=None):
    if random_state is None:
        rng = np.random
    elif isinstance(random_state, int):
        rng = np.random.RandomState(random_state)
...
def bbob_pen_func(x, ub):
    return np.sum(np.maximum(0.0, np.abs(x) - ub) ** 2, axis=-1)
