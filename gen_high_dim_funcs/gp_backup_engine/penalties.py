...
def compute_penalty(tree: ExpressionTree,
                    ...
                    w_depth: float,
                    w_dim: float,
                    w_symbol_cost: float) -> float:
    total = 0.
    total += w_len * length_penalty(tree) + w_depth * depth_penalty(tree) + w_dim * dimension_penalty(tree) + w_symbol_cost * symbol_cost_penalty(tree)
    return total
